...
<header id="top"> <!--头部通栏-->
	...
	<div class="container">
		<div class="upload_title">
		    ...
		    <p class="title-text">Update Goods</p>
		</div>
		<div class="attention col-lg-4" style="height:720px;">
				<div id="accordion">
					<div class="card">
			...
			<div class="upload_form" id="app">
				<form action="updateGoods.do" method="post" enctype="multipart/form-data">
					<div class="form_in">
						...
						<h4>商品名称</h4>
						<input type="text" name="g_name" id="g_name" required="required" value="${goods.g_name }"/>
						<h4>商品图片</h4>
						<img src="${goods.url }" style="width:100px; height:100px;"/>(图片路径:${goods.url })
						<h4>上传图片</h4>
						<input type="file" name="file" id="file">
						<h4>商品分类</h4>
						<input type="text" name="showt_id" id="showt_id" required="required" value="${goods.type.t_name }" disabled="disabled"/>
						选择分类:<select name="t_id" id="t_id">
							<c:forEach items="${list}" var="t">
								<option value="${t.t_id}">${t.t_name}</option>
							</c:forEach>
						</select>
						<h4>商品价格</h4>
						<input type="text" name="g_price" id="g_price" required="required"  value="${goods.g_price }"/>
						<h4>商品描述</h4>
						<textarea rows="10" cols="67" name="g_detail" id="g_detail">${goods.g_detail }</textarea>
						<br><br>
					</div>
